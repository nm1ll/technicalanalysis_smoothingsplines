...
for(i in 1:(length(data$close)-35-30)){
  ...
  if (numberExtremeValue>4){
    if (E1>E2){
      lowest_top <- min(c(E1,E3,E5))
      highest_bottom <- max(c(E2,E4))
    }
    if (E1<E2){
      lowest_top <- min(c(E2,E4))
       highest_bottom <- max(c(E1,E3,E5))
     }
  }
  
  if (numberExtremeValue>4){
    if (E1 > E2 &
        E1 > 0.9925*((E1+E3+E5)/3) & E3 > 0.9925*((E1+E3+E5)/3) & E5 > 0.9925*((E1+E3+E5)/3) &
        E1 < 1.0075*((E1+E3+E5)/3) & E3 < 1.0075*((E1+E3+E5)/3) & E5 < 1.0075*((E1+E3+E5)/3) &
        E2 > 0.9925*((E2+E4)/2) & E4 > 0.9925*((E2+E4)/2) & 
        E2 < 1.0075*((E2+E4)/2) & E4 < 1.0075*((E2+E4)/2) &
