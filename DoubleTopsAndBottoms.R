...
for(i in 1:(length(data$close)-35-30)){
  ...
  numberExtremeValue<-length(which((turning_points != 0) & (turning_points != 1) & (turning_points != 13)))
  ExtremeValue<-which((turning_points != 0) & (turning_points != 1) & (turning_points != 13))
  index<-which.max(btcsmooth$y[1:13])
  E1 <- 0
  if(index %in% ExtremeValue){
    E1 <- btcsmooth$y[index]
  }
  Ea <- max(btcsmooth$y[c((index+1):(index+22))])

  if (E1>0){
    if (
      E1 > 0.985*mean(c(E1,Ea)) & Ea > 0.985*mean(c(E1,Ea)) &
      E1 < 1.015*mean(c(E1,Ea)) & Ea < 1.015*mean(c(E1,Ea))
    )
    {
...
for(i in 1:(length(data$close)-35-30)){
  ...
  numberExtremeValue<-length(which((turning_points != 0) & (turning_points != 1) & (turning_points != 13)))
  ExtremeValue<-which((turning_points != 0) & (turning_points != 1) & (turning_points != 13))
  index<-which.min(btcsmooth$y[1:13])
  E1 <- 0
  if(index %in% ExtremeValue){
    E1 <- btcsmooth$y[index]
  }
  Eb <- min(btcsmooth$y[c((index+1):(index+22))])
  
  if (E1>0){
    if (
      E1 > 0.985*mean(c(E1,Eb)) & Eb > 0.985*mean(c(E1,Eb)) &
      E1 < 1.015*mean(c(E1,Eb)) & Eb < 1.015*mean(c(E1,Eb))
    )
    {
      min1_return <- round((price_trade[c(2)]-price_trade[c(2-1)])/price_trade[c(2-1)],4)
      min2_return <- round((price_trade[c(2+1)]-price_trade[c(2-1)])/price_trade[c(2-1)],4)
